...
function setupEventListeners() {
  ...
  // Classification simulator button click
  const btnRunSim = document.getElementById('btn-run-sim');
  if (btnRunSim) {
    btnRunSim.addEventListener('click', () => {
      const title = (document.getElementById('sim-title').value || '').trim();
      const author = (document.getElementById('sim-author').value || '').trim();
      const publisher = (document.getElementById('sim-publisher').value || '').trim();
      const series = (document.getElementById('sim-series').value || '').trim();
      
      if (!title) {
        alert('検証には少なくとも「書籍タイトル」の入力が必要です。');
        return;
      }
      ...
      let category = 'book'; // Default to 一般書
      let reason = '標準のフォールバック（一般書に該当）';
      ...
      if (hasShinshoKeyword) {
        category = 'shinsho';
        reason = '出版社、シリーズ名、またはタイトルに「新書」「選書」のキーワードを検知';
      } else {
        // Common non-fiction keywords
        ...
        const isKnownNovelist = novelists.some(auth => authorUpper && authorUpper.includes(auth.toUpperCase()));
        ...
        const isLiteraryPublisher = ['新潮社', '講談社', '集英社', '文藝春秋', '幻冬舎', 'ポプラ社', '双葉社', '角川', 'KADOKAWA', '徳間書店', '光文社', '早川書房', '東京創元社', '文春', '実業之日本社', 'ポプラ文庫', '宝島社'].some(pub => pubUpper && pubUpper.includes(pub));
        
        if (isKnownNovelist) {
          category = 'novel';
          reason = `登録作家リスト（${author}）に一致`;
        } else if (isFictionSeries && !isNonFictionSeries && !isNonFiction) {
          category = 'novel';
          reason = `文庫・ミステリレーベル（${series || publisher || '文庫'}）を検知し、ノンフィクションキーワード除外に不一致`;
        } else if (isLiteraryPublisher && !isNonFiction && !isNonFictionSeries) {
          category = 'novel';
          reason = `主要文芸出版社（${publisher}）を検知し、ノンフィクションキーワード除外に不一致`;
        } else {
          if (isNonFictionSeries) {
            reason = `学術・ノンフィクション専門レーベル（${series || publisher}）を検知`;
          } else if (hasNonFictionTitle) {
            reason = `タイトル内のノンフィクションキーワード「${matchedKw}」を検知`;
          } else if (isNonFictionAuthor) {
            reason = `ノンフィクション著者・評論家リスト（${author}）に一致`;
          }
        }
      }
      
      const resCategoryEl = document.getElementById('sim-result-category');
      const resReasonEl = document.getElementById('sim-result-reason');
      const resContainer = document.getElementById('sim-result');
      
      let catText = '一般書';
      if (category === 'novel') catText = '小説';
      if (category === 'shinsho') catText = '新書';
      
      resCategoryEl.textContent = catText;
      resReasonEl.innerHTML = `<strong>理由 / トリガー:</strong> ${reason}`;
